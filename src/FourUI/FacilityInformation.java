...
//	         dataset2.setValue(list.get(i).getDivision(), list.get(i).getUsage());
	      ...
	      //그래프를 그리는 객체 생성
	      //1.그래프의 제목, 2.x축제목, 3.y축제목, 4.그래프의 데이터, 5.그래프의 종류(위아래왼오), 6.범례사용여부, 7.툴팁사용여부, 8.확대사용여부
	      JFreeChart barChart = ChartFactory.createAreaChart("시설 평균점수", "  ", "점수", dataset1, PlotOrientation.VERTICAL, false, true, true );
//	      
	      JFreeChart piechart = ChartFactory.createPieChart("기간별 사용량", dataset2, true, true, true);
	      //한글 인코딩
	      //한글 글씨체를 저장할 객체 생성
	      //1. 폰트종류, 2.폰트스타일, 3.크기
	      Font f = new Font("함초롱바탕", Font.PLAIN, 14);
	      //그래프 제목에 한글 적용
